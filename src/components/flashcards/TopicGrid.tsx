...
export function TopicGrid({ cards, userId, onRefresh }: TopicGridProps) {
  ...
  const topics = useMemo(() => {
    ...
    return Object.entries(groups).sort(([, aCards], [, bCards]) => {
      const avgA =
        aCards.reduce((acc, c) => acc + c.confidence_level, 0) / aCards.length;
      const avgB =
        bCards.reduce((acc, c) => acc + c.confidence_level, 0) / bCards.length;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in zoom-in duration-500">
        ...
        <p className="text-gray-400 max-w-md mb-8">
          You have no active misconceptions pending review. Keep taking quizzes
          to find new ones!
        </p>
        <button
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400">
              {activeTopicCards.length} cards in this deck
            </p>
          </div>
          <button
          ...
          {activeTopicCards.map((card, index) => (
            ...
            >
              ...
              <div className="relative z-10">
                <div className="mb-3">
                  <span
                    className={`inline-block px-2 py-1 rounded-md text-[10px] uppercase font-bold tracking-wider
                                    ${
                                      card.card_type === "misconception_breaker"
                                        ? "bg-orange-900/30 text-orange-400"
                                        : card.card_type === "prediction"
                ...
                <div className="w-full h-1.5 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      card.confidence_level < 2
                        ? "bg-red-500"
                        : card.confidence_level > 4
                          ? "bg-green-500"
                          : "bg-blue-500"
                    }`}
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {topics.map(([topic, topicCards], index) => {
          ...
          const avgConf =
            topicCards.reduce((acc, c) => acc + c.confidence_level, 0) / count;
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10">
                <div
                  className={`flex justify-between items-start mb-6 ${isWeak ? "mt-8" : ""}`}
                >
                  <div
                    className={`p-4 rounded-2xl shadow-inner ${
                      isWeak
                        ? "bg-gradient-to-br from-red-500 to-orange-600 text-white shadow-red-900/50"
                        : isStrong
                          ? "bg-gradient-to-br from-green-400 to-emerald-600 text-white shadow-green-900/50"
